...
def billItem():
    ...
    global guestName
    guestName = StringVar()
    id2 = Entry(bill, textvariable=guestName).grid(row=5, column=1, padx=20, pady=5)
    ...
    callback1 = bill.register(only_Alpha)  # registers a Tcl to Python callback
    id2.config(validate="key", validatecommand=(callback1, "%P"))  # enables validation
...
def displayBill():
    ...
    query=f"select * from guest_bill WHERE TimeAndDate='{a}';"
    cursor.execute(query)
    data=cursor.fetchall()
    for row in data:
